...
Right
```bash
west build  -d build/ritemper/right -b nice_nano_v2 -- -DSHIELD=ritemper_right -DZMK_CONFIG=/workspaces/zmk-config/config  -DZMK_EXTRA_MODULES="/workspaces/zmk-modules/cirque-input-module;/workspaces/zmk-config"
```

Left Central 
```bash
west build --pristine  -d build/ritemper/left -b nice_nano_v2 -- -DSHIELD="ritemper_left nice_view_adapter nice_view" -DZMK_CONFIG=/workspaces/zmk-config/config  -DZMK_EXTRA_MODULES="/workspaces/zmk-modules/cirque-input-module;/workspaces/zmk-config"
```
### Needed Module:
